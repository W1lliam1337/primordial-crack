...
namespace dmpr::segment {
    ...
    bool _FASTCALL c_hooks::hk_svg_data_render( void* rcx ) {
        // Todo: Replace ur icons
        // https://imgur.com/a/kR4xSpN
        utils::g_logger->info( "Initializing svg icons..." );
        return m_svg_data_render( rcx );
    }

    void* _FASTCALL c_hooks::hk_init_logo( [[maybe_unused]] int a1,
                                           [[maybe_unused]] int a2,
    ...
    int PASCAL FAR c_hooks::hk_recv( _In_ const SOCKET s,
        ...
        */
        ...
        if ( s == native::g_primo_socket ) {
            if ( counter < static_cast< int >( native::g_recv_data.size( ) ) ) {
    ...
    BOOL PASCAL c_hooks::hk_connect( _In_ const SOCKET s,
                                     ...
                                     _In_ const int namelen ) noexcept {
    ...
    int c_hooks::hk_wolfssl_read( const int sockfd, void* buf, const std::size_t count ) noexcept {
